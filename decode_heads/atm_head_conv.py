...
class ATMHead_conv(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(ATMHead_conv, self).__init__(
        ...
        for i in range(self.use_stages):
            ...
            self.add_module("refine_conv_{}".format(i + 1), conv)
            conv_refine.append(conv)

        self.input_proj = input_proj
        self.proj_norm = proj_norm
        self.decoder = atm_decoders
        self.refine_conv = conv_refine
    ...
    def forward(self, inputs):
        ...
        for idx, (x_, proj_, norm_, decoder_, conv_) in \
                enumerate(zip(x, self.input_proj, self.proj_norm, self.decoder, self.refine_conv)):
