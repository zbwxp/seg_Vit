...
class ATMHead_crop(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(ATMHead_crop, self).__init__(
        ...
        self.pos_emb = PositionEmbeddingSine(dim//2, normalize=True)
    ...
    def forward(self, inputs):
        ...
        for idx, (x_, proj_, norm_, decoder_, decoder_full_, encoder_) in \
            ...
            # fill new q
            mask_tokens = self.mask_token.repeat(lateral.shape[0], (self.image_size//16)**2, 1)
            mask_tokens[:, inputs[-1]] = lateral
            x_ = self.d3_to_d4(mask_tokens)
            # add pos embedding
            pos = self.pos_emb(x_)
            ...
            qs.append(q_full.transpose(0, 1))
            attns.append(attn)
        qs = torch.stack(qs, dim=0)
        outputs_class = self.class_embed(qs)
        out = {"pred_logits": outputs_class[-1]}

        outputs_seg_masks = attns
        size = maps_size[-1]
