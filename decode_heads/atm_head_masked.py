...
class TPN_DecoderLayer(TransformerDecoderLayer):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        tgt2 = self.self_attn(tgt, tgt, tgt, attn_mask=None,
                              key_padding_mask=tgt_key_padding_mask)[0]
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)
        tgt2, attn2 = self.multihead_attn(
            tgt.transpose(0, 1), memory.transpose(0, 1), memory.transpose(0, 1), tgt_mask)
...
class ATMHead_masked(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(ATMHead_masked, self).__init__(
    ...
    def forward(self, inputs):
        ...
        for idx, (x_, proj_, norm_, decoder_) in enumerate(zip(x, self.input_proj, self.proj_norm, self.decoder)):
            ...
            q, attn = decoder_(q, lateral.transpose(0, 1), mask)
            attn = attn.transpose(-1, -2)
            attn = self.d3_to_d4(attn)
            maps_size.append(attn.size()[-2:])
            qs.append(q.transpose(0, 1))
            attns.append(attn)
            if idx < self.use_stages - 1:
                mask = F.interpolate(attn, scale_factor=2, mode="bilinear", align_corners=False)
                mask = (mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1) < 0.5).bool()
                mask = mask.detach()
