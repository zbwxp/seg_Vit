...
class TPNATMHead_noinit(BaseDecodeHead):
    def __init__(
            self,
            img_size,
            in_channels,
        ...
        super(TPNATMHead_noinit, self).__init__(
    ...
    def forward(self, inputs):
        ...
        for idx, (x_, proj_, norm_) in \
                enumerate(zip(x, self.input_proj, self.proj_norm)):
            lateral = norm_(proj_(x_))
            laterals.append(lateral)

        q = self.decoder(laterals[0].transpose(0, 1), laterals[1].transpose(0, 1))
